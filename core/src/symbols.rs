...
use crate::patterns;
use crate::spec::{SegmentType, SymbolSpec};
...
pub fn resolve_in_exe(
    ...
    for list2 in list.as_array().unwrap() {
        for value in list2.as_array().unwrap() {
            if value["symbol"].is_string() {
                refs.insert(
                    format_name_for_addr(value["symbol"].as_str().unwrap().into()),
                    vec![u64::from_str_radix(value["offset"].as_str().unwrap(), 16).unwrap() + 0x1000],
                );
            ...
            if value["hash"].is_string() {
                ...
                hashes.insert(
                    value["hash"].as_str().unwrap().parse::<u64>().unwrap(),
    ...
    for value in list.as_array().unwrap() {
        refs.insert(
            format_name_for_addr(value["symbol"].as_str().unwrap().into()),
            vec![u64::from_str_radix(value["offset"].as_str().unwrap(), 16).unwrap()],
        );
        syms.push(FunctionSymbol::new(
            value["symbol"].as_str().unwrap().into(),
            value["symbol"].as_str().unwrap().into(),
    ...
    for string in strings {
        ...
        v.push(0);
        let addrs: Vec<u64> = patterns::single_search(vec![v], exe.rdata())
            .iter()
            .map(|x| x + exe.rdata_offset_from_base())
            .collect();
    ...
    refs.insert(
        "null".to_owned(),
        patterns::single_search(vec![vec![0xC2, 0x00, 0x00, 0xCC]], exe.text())
            .iter()
            .map(|x| x + exe.text_offset_from_base())
            .collect(),
    );
    ...
    refs.insert(
        "ret0".to_owned(),
        patterns::single_search(
            vec![vec![0x32, 0xC0, 0xC3, 0xCC], vec![0x33, 0xC0, 0xC3, 0xCC], vec![
                0xC2, 0x00, 0x00, 0xCC,
            ]],
            exe.text(),
        )
    ...
    for mut spec in specs {
        match spec.segment {
            SegmentType::Any | SegmentType::Text => {
                if let Some(Some(addr)) = spec.hash.map(|hash| hashes.get(&hash)) {
                    spec.addrs.push(*addr + exe.text_offset_from_base());
                    all_specs.push(spec);
                } else {
            ...
            SegmentType::IData => {
                if let Some(Some(addr)) = spec.hash.map(|hash| hashes.get(&hash)) {
                    spec.addrs.push(*addr + exe.idata_offset_from_base());
                    all_specs.push(spec);
                } else {
            ...
            SegmentType::RData => {
                if let Some(Some(addr)) = spec.hash.map(|hash| hashes.get(&hash)) {
                    spec.addrs.push(*addr + exe.rdata_offset_from_base());
                    all_specs.push(spec);
                } else {
            ...
            SegmentType::Data => {
                if let Some(Some(addr)) = spec.hash.map(|hash| hashes.get(&hash)) {
                    spec.addrs.push(*addr + exe.data_offset_from_base());
                    all_specs.push(spec);
                } else {
            ...
            SegmentType::PData => {
                if let Some(Some(addr)) = spec.hash.map(|hash| hashes.get(&hash)) {
                    spec.addrs.push(*addr + exe.pdata_offset_from_base());
                    all_specs.push(spec);
                } else {
    ...
    for (addrs, mut spec) in patterns::multi_search_ref(
        text_specs.iter().filter_map(|x| x.pattern.as_ref()),
        exe.text(),
        exe.text_offset_from_base(),
        &refs,
    )
    .iter()
    .zip(text_specs)
    ...
    for (addrs, mut spec) in patterns::multi_search_ref(
        rdata_specs.iter().filter_map(|x| x.pattern.as_ref()),
        exe.rdata(),
        exe.rdata_offset_from_base(),
        &refs,
    )
    .iter()
    .zip(rdata_specs)
    ...
    for (addrs, mut spec) in patterns::multi_search_ref(
        data_specs.iter().filter_map(|x| x.pattern.as_ref()),
        exe.data(),
        exe.data_offset_from_base(),
        &refs,
    )
    .iter()
    .zip(data_specs)
...
fn resolve_symbol(spec: &SymbolSpec, data: &ExecutableData, rva: u64) -> Result<FunctionSymbol> {
    let res = match &spec.eval {
        Some(expr) => {
            expr.eval(&EvalContext::new(spec.pattern.as_ref().unwrap(), data, data.rel_offset(rva))?)? - data.image_base()
        }
