...
pub fn format_name_for_addr(s: &FunctionSymbol) -> String {
...
pub fn write_c_header<W: Write>(mut output: W, symbols: &[FunctionSymbol], message: &Option<String>) -> Result<()> {
    writeln!(output, "{}", HEADER)?;
    if let Some(errors) = message {
    ...
    for symbol in sorted {
        ...
        //     symbol.rva()
        // )?;
        let addr_name = format_name_for_addr(&symbol);
        writeln!(
            output,
            "#ifndef {}_Addr\n#define {}_Addr 0x{:X}\n#endif",
            addr_name,
...
pub fn write_c_definition<W: Write>(mut output: W, symbols: &[FunctionSymbol], message: &Option<String>) -> Result<()> {
    writeln!(output, "#pragma once")?;
    write_c_header(output.by_ref(), symbols, message)?;
